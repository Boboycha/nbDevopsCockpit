...
(* Регистрация вызывается явно из nbCodeEditor.pas, а не из initialization
   этого юнита: юнит нигде не референсится напрямую (только через строковый
   language id в реестре), и cross-platform smart-linker (в частности на
   Linux) может посчитать его неиспользуемым и выкинуть из сборки вместе
   с initialization-секцией. Явный вызов из гарантированно слинкованного
   юнита исключает этот риск. *)
procedure RegisterPascalSyntax;
...
begin
  ...
  KeyWord.Color := TCodeSyntax.Palette.KeywordControl;
  KeyWord.Font.Assign(FDefaultFont);
  KeyWord.Font.Style := [TFontStyle.fsBold];
  FKeyWords.Add(KeyWord);
  ...
  FNumKey.Color := TCodeSyntax.Palette.NumberColor;
  ...
  FDirectiveKey.Color := TCodeSyntax.Palette.DirectiveColor;
...
procedure RegisterPascalSyntax;
begin
  TCodeSyntax.RegisterSyntax(['pascal', 'delphi', 'fpc', 'freepascal', 'free-pascal', 'pas'], TCodeSyntaxPascal);
end;
