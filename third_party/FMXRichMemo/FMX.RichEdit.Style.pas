...
type
  ...
  private
    ...
    FColorCaret: TAlphaColor;
    procedure SetColorCaret(const Value: TAlphaColor);
    ...
    procedure DrawCaretOverlay(const ACanvas: TCanvas);
  ...
  public
    ...
    property ColorCaret: TAlphaColor read FColorCaret write SetColorCaret;
    // Gutter
...
begin
  ...
  if not (TCorner.BottomLeft in Corners) then
  begin
    var R := ARegion;
  ...
  if not (TCorner.BottomRight in Corners) then
  begin
    var R := ARegion;
  ...
  if not (TCorner.TopRight in Corners) then
  begin
    var R := ARegion;
    R.Offset(ARegion.Width - 3, -ARegion.Height + 7);
    R.Width := 6;
    ...
    for var i := Max(0, Editor.LinesLayout.FirstVisibleLineIndex) to Min(Editor.LinesLayout.LastVisibleLineIndex, Editor.LinesLayout.Count - 1) do
      ...
      begin
        ...
        begin
          ...
          ACanvas.FillRect(Rect, TAlphaColorRec(FColorCurrentLine).A / 255.0);
...
begin
  inherited;
  if FColorCaret <> $00000000 then
    Memo.Caret.Color := FColorCaret;
  var Content: TLayout;
...
begin
  ...
  DrawCaretOverlay(ACanvas);
end;

procedure TRichEditStyled.DrawCaretOverlay(const ACanvas: TCanvas);
var
  Flasher: IFlasher;
  R: TRectF;
begin
  (* FMX вызывает OnPaint контрола после PaintChildren, поэтому контент,
     нарисованный здесь (подсветка текущей строки и текст), закрашивает
     каретку-flasher — ребёнка scroll content. Дорисовываем каретку
     поверх контента вручную; мигание сохраняется, так как изменение
     Opacity flasher'а инвалидирует его регион и OnPaint выполняется
     повторно. *)
  if (Memo.Caret = nil) or not Memo.Caret.Displayed then
    Exit;
  Flasher := Memo.Caret.Flasher;
  if (Flasher = nil) or (Flasher.Caret <> Memo.Caret) or
    not Flasher.Visible or (Flasher.Opacity < 0.01) then
    Exit;
  R := TRectF.Create(Flasher.Pos, Max(Flasher.Size.cx, 1), Flasher.Size.cy);
  ACanvas.Fill.Kind := TBrushKind.Solid;
  if Flasher.Color <> TAlphaColorRec.Null then
    ACanvas.Fill.Color := Flasher.Color
  else
    ACanvas.Fill.Color := TAlphaColorRec.Black;
  ACanvas.FillRect(R, 0, 0, AllCorners, Flasher.Opacity);
end;
...
procedure TRichEditStyled.SetColorCaret(const Value: TAlphaColor);
begin
  FColorCaret := Value;
  Memo.Caret.Color := Value;
  Repaint;
end;
