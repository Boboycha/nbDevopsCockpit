...
type
  ...
  // Возвращает разделяемую (interned) строку для однобайтового печатного
  // ASCII-символа вместо новой heap-аллокации на каждую ячейку. В обычном
  // текстовом выводе (логи, код, шелл) это подавляющее большинство ячеек
  // терминала - каждая такая ячейка сегодня платит за отдельный heap-блок
  // под 1-символьную строку. Non-ASCII и составные графемы (эмодзи, ZWJ,
  // комбинирующие знаки) возвращаются как есть, без изменений.
  function InternAsciiChar(const Ch: string): string;
...
// --- INTERNING ОДНОСИМВОЛЬНЫХ ASCII-СТРОК ---

var
  (* Индексы 32..126 - печатные ASCII; 0-й элемент не используется. *)
  AsciiInternTable: array [0 .. 126] of string;

procedure InitAsciiInternTable;
var
  Code: Integer;
begin
  for Code := 32 to 126 do
    AsciiInternTable[Code] := Chr(Code);
end;
...
begin
  ...
  begin
    ...
    if (Code >= 32) and (Code <= 126) then
      Exit(AsciiInternTable[Code]);
  end;
  Result := Ch;
end;

initialization
  InitAsciiInternTable;
